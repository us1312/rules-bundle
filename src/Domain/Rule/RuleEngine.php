...
final readonly class RuleEngine {
    ...
    public function __construct(private RuleRegistry $registry, private EntityManagerInterface $em) {}
    ...
    public function handle(EventInterface $event): void {
        ...
        foreach ($rules as $rule) {
            ...
            foreach ($rule->getConditions() as $condition) {
                ...
                if ($conditionObject instanceof RequiresContext) {
                    $missing = array_diff($conditionObject->getRequiredContextKeys(), array_keys($context));
                    if ($missing) {
                        $allMet = false;
                        break;
                    }
                }
                if (!$conditionObject->evaluate([
                    ...$context,
                    ...$rule->getParameters(),
            ...
            foreach ($rule->getActions() as $action) {
                ...
                if ($actionObject instanceof RequiresContext) {
                    $missing = array_diff($actionObject->getRequiredContextKeys(), array_keys($context));
                    if ($missing) {
                        continue;
                    }
                }
